...
export class EquipItemUseCase {
    ...
    async execute(command: EquipItemCommand): Promise<TacticalCharacter> {
        ...
        return await this.tacticalCharacterRepository.update(command.characterId, character);
    }

    private validateEquipmentData(character: TacticalCharacter, command: EquipItemCommand): void {
    ...
    private applyEquipmentLogic(character: TacticalCharacter, command: EquipItemCommand): void {
        const item = character.items.find((e: any) => e.id === command.itemId);
        if (!item) {
